...
import { PATHS } from "@test-data/constants";
import { MESSAGES, createMessage } from "@test-data/messages";
...
test.describe("Blog API", () => {
  ...
  test("GET /api/blog/year/:year works", async ({ api }) => {
  ...
  test("GET /api/blog/:slug returns 404 for unknown slug", async ({ api }) => {
